...
from transformers.utils import replace_return_docstrings, add_start_docstrings, add_start_docstrings_to_model_forward
...
class MistralModelWithQuietSTaR(MistralModel):
    def forward(
            self,
...
class MistralForCausalLMWithQuietSTaR(MistralPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = MistralModelWithQuietSTaR(config)
    ...
    def infer(
        ...
        input_ids = torch.cat([input_ids, torch.tensor([[start_thought_token_id]] * batch_size).to(input_ids.device)],
                              dim=-1)
        seq_len += 1
        ...
        for continuation_idx in range(continuation_length):
            ...
            # Update the attention mask
            if attention_mask is not None:
                attention_mask = torch.cat([attention_mask, torch.ones((batch_size, 1)).to(attention_mask.device)],
                                           dim=-1)

        # Append the end thought token to the input sequence
        end_thought_token_id = self.tokenizer.convert_tokens_to_ids("<|endthought|>")
        input_ids = torch.cat([input_ids, torch.tensor([[end_thought_token_id]] * batch_size).to(input_ids.device)],
                              dim=-1)
        seq_len += 1
        ...
        outputs_after = self.model(
            input_ids=torch.cat([next_token_id.unsqueeze(-1).to(input_ids.device),
    ...
    def forward(
        ...
        if self.tokenized_thought_prefix is None and self.use_thought_prefix:
            self.tokenized_thought_prefix = \
                self.tokenizer(self.thought_prefix, return_tensors="pt", add_special_tokens=False)["input_ids"]
        ...
        # multiple computational cycle of self.n_passed time
        if self.n_passes > 1:
        ...
        if self.start_token_id is None:
            ...
            if self.start_token_id == 0:
                ...
                if self.initialize_thought_embedding_to_normal:
                    ...
                    self.start_embedding.data[0] = self.model.embed_tokens.weight.data[
                                                       base_start_id].clone().detach() / self.embedding_scale
                self.start_embedding.data[1] = torch.log(
                    self.model.embed_tokens.weight.data.std(dim=0) * self.thought_init_std_scale / self.embedding_scale)
        if self.end_token_id is None:
            ...
            if self.end_token_id == 0:
                ...
                if self.initialize_thought_embedding_to_normal:
                    ...
                    self.end_embedding.data[0] = self.model.embed_tokens.weight.data[
                                                     base_end_id].clone().detach() / self.embedding_scale
                self.end_embedding.data[1] = torch.log(
                    self.model.embed_tokens.weight.data.std(dim=0) * self.thought_init_std_scale / self.embedding_scale)
        ...
        if not self.rm_initialized and (self.n_ahead > 1 or not self.base_original_mode):
            ...
            if not self.use_shallow_talk:
                ...
                cur_head.weight.data = torch.zeros(talk_output_dim, talk_input_dim, device=cur_head.weight.device,
        ...
        for ahead_idx in range(fwd_iters):
            ...
            if inputs_embeds is None:
                ...
                if contains_thought:
                    ...
                    if self.use_reparam_for_thought_embeddings:
                        inputs_embeds = torch.randn(batch_size, seq_len, self.model.config.hidden_size,
                                                    device=input_ids.device, dtype=cur_thought_embedding.dtype)
                        inputs_embeds = inputs_embeds.detach() * torch.exp(cur_thought_embedding[1]) + \
                                        cur_thought_embedding[0]
                        if contains_start:
                ...
                if attention_mask is None:
                    ...
                    if seq_len + past_key_values_length != attention_mask.shape[-1]:
                        ...
                        attention_mask = torch.cat(
                            [torch.ones((attention_mask.shape[0], past_key_values_length), dtype=attention_mask.dtype,
            ...
            attempted = False
            talk_loss_list = []
            if self.original_mode or (self.n_ahead == 1) or (
                    self.comparison_mode and ahead_idx == 0):  # or (self.optimize_lm_head_only_at_start and ahead_idx == 0):
                loss = None
                attempted = True
                ...
                if labels is not None:
                    for shift_amount in range(self.n_ahead_talk):
                        ...
                        if not self.comparison_mode and not (self.optimize_lm_head_only_at_start and (
                                self.n_ahead + self.n_ahead_talk > 2)) or self.original_mode:
            ...
            if not attempted or self.comparison_mode:
                ...
                if override_token is not None and self.n_ahead > 1:
                    ...
                    if labels is not None:  # we're in the talk phase
                        ...
                        valid_shift_labels = torch.where(shift_labels == -100, torch.tensor(self.tokenizer.pad_token_id,
                        ...
                        # )
                        new_rm_tokens = torch.cat(
                            [valid_shift_labels, padding],
                            dim=-1
                        )
                        # convert rm tokens to one-hot
                        probabilities_2d = F.one_hot(new_rm_tokens, num_classes=self.vocab_size).reshape(-1,
                                                                                                         self.vocab_size).to(
                            probabilities_2d.dtype)
                        skip_sampling = True
                    else:
                        continue
                ...
                if not contains_thought:
                    ...
                    if self.use_reparam_for_thought_embeddings:
                        ...
                        inputs_embeds = cur_thought_embedding.unsqueeze(0).repeat(batch_size, seq_len, 1)
                        inputs_embeds = inputs_embeds.view(probabilities.size(0), probabilities.size(1), -1).to(
                ...
                if labels is not None and (self.n_ahead > 1 or not self.base_original_mode):
                    ...
                    if self.use_policy_loss and ahead_idx > 0 and (ahead_idx > 1 or not self.use_start_thought_token):
                        ...
                        if not did_skip_sampling:
                            ...
                            action_loglikelihoods = \
                                F.log_softmax(prev_sample_probs / self.reinforce_temperature, dim=-1)[
                    ...
                    if self.use_policy_loss and ahead_idx == self.n_ahead + self.n_ahead_talk - 2:
                        ...
                        if self.use_reparam_for_thought_embeddings and (
                                self.use_start_thought_token or self.use_end_thought_token):
                            ...
                            if self.use_start_thought_token:
                                ...
                                start_loglikelihood = -0.5 * (
                                        sampled_start.detach() - start_embedding[0]) ** 2 / exp_start_std ** 2 - \
                                                      start_embedding[1] - 0.5 * math.log(2 * math.pi)
                                start_loglikelihood = start_loglikelihood.mean(dim=-1)
                            if self.use_end_thought_token:
                                exp_end_std = torch.exp(end_embedding[1])
                                end_loglikelihood = -0.5 * (
                                        sampled_end.detach() - end_embedding[0]) ** 2 / exp_end_std ** 2 - \
                                                    end_embedding[1] - 0.5 * math.log(2 * math.pi)
                        ...
                        if ahead_idx == self.n_ahead + self.n_ahead_talk - 2 and self.eval_mode:
                            with torch.no_grad():
                                ...
                                filtered_tokens = input_ids[:,
                                                  :policy_reward.shape[-1]].cpu().detach().numpy().flatten()
                                filtered_tokens_mask = filtered_tokens != self.tokenizer.pad_token_id
                                filtered_tokens = filtered_tokens[filtered_tokens_mask]
                                filtered_rewards = policy_reward.float().cpu().detach().numpy()[:,
                                                   :seq_len - self.n_ahead_talk].flatten()
                                filtered_rewards = filtered_rewards[filtered_tokens_mask]

                                abs_reward_list = np.abs(policy_reward.float().cpu().detach().numpy()[:,
                                                         :seq_len - self.n_ahead_talk].flatten())
                                ...
                                def plot_kde(data, losses):
                                    ...
                                    base_colors = sns.color_palette("light:#5A9",
                                                                    n_colors=256)  # More colors for a smoother gradient
                                    base_cmap = LinearSegmentedColormap.from_list("log_light", base_colors)
                                    log_norm = LogNorm(vmin=1e-3, vmax=10)

                                    sns.kdeplot(x=data, y=losses, fill=True, levels=20, norm=log_norm, cut=0,
                                                linewidths=0)
                                ...
                                self.all_unreduced_losses.extend(unreduced_loss[:, :-1].flatten()[
                                                                     filtered_tokens_mask].float().flatten().cpu().detach().numpy())
                        ...
                        for action_loglikelihoods_2d in action_loglikelihoods_list:
                            ...
                            if self.trice_mode and self.n_passes > 1:
                                batched_policy_reward = train_policy_reward.reshape(-1, self.n_passes,
                                                                                    train_policy_reward.shape[-1])
                                # average over the passes
                                train_policy_reward = batched_policy_reward - batched_policy_reward.mean(dim=1,
                                                                                                         keepdim=True)
                            ...
                            actor_loss = -fixed_policy_reward * action_loglikelihoods_2d[:,
                                                                :policy_reward.shape[-1]].to(policy_reward.device)
        ...
        if not loss_list:
            ...
            log_dict["loss_talk"] += sum(nonzero_mean(cur_loss_item) for cur_loss_item in
                                         loss_list[-self.n_ahead_talk:]) / self.n_ahead_talk / self.n_tokens_print
        ...
        try:
            ...
            if self.wandb_enabled:
                if self.training_steps % (
                        self.n_tokens_print) == 0 or not self.training:  # and "0" in str(loss.device):
                    if not self.training:
                        new_log_dict = {}
                    ...
                    if self.n_ahead > 1:
                        log_dict["compute_steps"] = self.training_steps * batch_size * (
                                self.n_ahead + self.n_ahead_talk - 1) * self.gradient_accumulation_steps
...
class MistralForQuietSTaR(MistralPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]
    ...
    def init_special_embedding(self, start_prompt, end_prompt, init_prompt, tokenizer):
        self.tokenizer = tokenizer
        self.start_token_id = tokenizer.convert_tokens_to_ids([start_prompt])[0]
        self.end_token_id = tokenizer.convert_tokens_to_ids([end_prompt])[0]
        init_token_id = tokenizer.convert_tokens_to_ids([init_prompt])[0]

        init_embedding = self.get_input_embeddings().weight[init_token_id]
        self.model.embed_tokens.weight.data[self.start_token_id] = init_embedding.clone()
        self.model.embed_tokens.weight.data[self.end_token_id] = init_embedding.clone()

    def non_repeat_interleave(self, x, n):
        if x is None:
            return x
        return x.repeat_interleave(n, dim=0)

    def generate_thoughts(self, inputs_embeds, attention_mask, thought_length, gumbel_temperature):
        batch_size, input_seq_len, dims = inputs_embeds.shape
        thought_embeds = self.model.embed_tokens(torch.tensor(self.start_token_id, device=inputs_embeds.device)).view(1,
                                                                                                                      1,
                                                                                                                      dims).repeat(
            batch_size,
            thought_length + 2, 1)

        for t in range(1, thought_length + 1):
            thought_embeds_inputs = torch.cat([inputs_embeds, thought_embeds[:, :t, :]], dim=1)
            thought_attention_mask = torch.cat([attention_mask,
                                                torch.ones((batch_size, t), dtype=inputs_embeds.dtype,
                                                           device=inputs_embeds.device)], dim=1)
            thought_logits = self.model(inputs_embeds=thought_embeds_inputs, attention_mask=thought_attention_mask)[0][
                             :,
                             -1:, :]
            thought_probs = F.gumbel_softmax(self.lm_head(thought_logits), tau=gumbel_temperature, hard=True, dim=-1)
            thought_embeds[:, t, :] = (thought_probs @ (self.model.embed_tokens.weight)).squeeze(1)

        thought_embeds[:, -1, :] = self.model.embed_tokens(torch.tensor(self.end_token_id, device=inputs_embeds.device))
    ...
    def forward(
        ...
        if self.n_thought > 1:
            input_ids = self.non_repeat_interleave(input_ids, self.n_thought)
            attention_mask = self.non_repeat_interleave(attention_mask, self.n_thought)
            position_ids = self.non_repeat_interleave(position_ids, self.n_thought)
            inputs_embeds = self.non_repeat_interleave(inputs_embeds, self.n_thought)
            labels = self.non_repeat_interleave(labels, self.n_thought)
            if past_key_values is not None:
                past_key_values = [self.none_repeat_interleave(p, self.n_thought) for p in past_key_values]

        if inputs_embeds is None:
            inputs_embeds = self.model.embed_tokens(input_ids)

        output_init = self.model(
            inputs_embeds=inputs_embeds,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
        ...
        log_p_talk_list = []
        hidden_state_init = output_init[0]
        total_loss = torch.tensor(0.0, dtype=hidden_state_init.dtype, device=hidden_state_init.device)
        if seq_len > self.n_true + 1:
            # shift in advance
            hidden_state_init = hidden_state_init[:, :-1, :].contiguous()
            labels = labels[:, 1:].contiguous()
            seq_len -= 1
            for j in range(seq_len - self.n_true - 1):
                if not (labels[:, j] != -100).any():
                    continue
                # predict base log prob
                h_init = hidden_state_init[:, j + 1: j + 1 + self.n_true, :].clone()
                log_p_init = F.log_softmax(self.lm_head(h_init), dim=-1)

                # generate thoughts
                thought_embeds = self.generate_thoughts(inputs_embeds[:, :j + 1, :], attention_mask[:, :j + 1],
                                                        self.thought_length, self.gumbel_temperature)
                thought_attention_mask = torch.ones((thought_embeds.shape[0], thought_embeds.shape[1]),
                                                    dtype=attention_mask.dtype, device=attention_mask.device)
                n_true_thought_embeds = torch.cat(
                    [inputs_embeds[:, :j + 1, :], thought_embeds, inputs_embeds[:, j + 1:j + 1 + self.n_true, :]],
                    dim=1)
                n_true_thought_attention_mask = torch.cat(
                    [attention_mask[:, :j + 1], thought_attention_mask, attention_mask[:, j + 1:j + 1 + self.n_true]],
                    dim=-1)

                # compute thought augmented hidden states and log prob
                h_thought = \
                    self.model(inputs_embeds=n_true_thought_embeds, attention_mask=n_true_thought_attention_mask)[0]
                n_true_thought = h_thought[:, -self.n_true:, :]
                log_p_thought = F.log_softmax(self.lm_head(n_true_thought), dim=-1)

                mixing_weights = torch.sigmoid(self.mix_head(torch.cat([h_init, n_true_thought], dim=-1)))
                log_p_talk = mixing_weights * log_p_thought + (1 - mixing_weights) * log_p_init

                # NLL loss
                if (labels[:, j + 1:j + self.n_true + 1] != -100).any():
                    nll_loss = F.nll_loss(log_p_talk.reshape(-1, log_p_talk.shape[-1]),
                                          labels[:, j + 1:j + self.n_true + 1].reshape(-1), ignore_index=-100)
                    total_loss += nll_loss
                else:
                    nll_loss = None
        ...
        # sep_len < self.n_true, shift into next token prediction
        else:
            lm_logits = self.lm_head(hidden_state_init)
        ...
        return CausalLMOutputWithPast(
            loss=total_loss,
            hidden_states=hidden_state_init,
        )
